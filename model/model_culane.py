import torch
from model.backbone import resnet, mobilenet_v3
import numpy as np
...
class parsingNet(torch.nn.Module):
    def __init__(self,
        ...
        self.cls = torch.nn.Sequential(
            ...
            if fc_norm else torch.nn.Identity(),
    ...
    def forward(self, x):
        ...
        pred_dict = {
            'loc_row':
            out[:, :self.dim1].view(-1, self.num_grid_row, self.num_cls_row,
                                    self.num_lane_on_row),
            'loc_col':
            out[:,
                self.dim1:self.dim1 + self.dim2].view(-1, self.num_grid_col,
                                                      self.num_cls_col,
                                                      self.num_lane_on_col),
            'exist_row':
            out[:,
                self.dim1 + self.dim2:self.dim1 + self.dim2 + self.dim3].view(
                    -1, 2, self.num_cls_row, self.num_lane_on_row),
            'exist_col':
            out[:, -self.dim4:].view(-1, 2, self.num_cls_col,
                                     self.num_lane_on_col)
        }
        if self.use_aux:
    ...
    def forward_tta(self, x):
        ...
        # 10 x 25
        fea = torch.cat([
            pooled_fea, left_pooled_fea, right_pooled_fea, up_pooled_fea,
            down_pooled_fea
        ],
                        dim=0)
        fea = fea.view(-1, self.input_dim)
...
def get_model(cfg):
    return parsingNet(pretrained=True,

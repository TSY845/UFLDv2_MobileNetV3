...
def soft_nll(pred, target, ignore_index = -1):
    ...
    # return torch.clamp(-(target_fusion * pred).sum() / (target!=ignore_index).sum(), min=1e-7 - 1, max=1e-7)

class SoftmaxFocalLoss(nn.Module):
    def __init__(self, gamma, ignore_lb=255, soft_loss = False, *args, **kwargs):
